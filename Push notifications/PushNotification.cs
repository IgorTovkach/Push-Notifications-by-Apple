...
namespace Push_notifications
{
    class PushNotification
    {
        ...
        private List<NotificationPayload> _notificationsList = new List<NotificationPayload>();
        ...
        public PushNotification(bool useSandbox, string p12File, string p12FilePassword)
        {
            ...
            //Load collection of certificates
        ...
        public List<string> SendToApple(List<NotificationPayload> queue)
        {
            ...
            _notificationsList = queue;
            if (queue.Count < 8999)
            {
                SendQueueToapple(_notificationsList);
            }
            else
            {
                const int pageSize = 8999;
                var numberOfPages = (queue.Count/pageSize) + (queue.Count%pageSize == 0 ? 0 : 1);
                var currentPage = 0;

                while (currentPage < numberOfPages)
                {
                    _notificationsList = (queue.Skip(currentPage*pageSize).Take(pageSize)).ToList();
                    SendQueueToapple(_notificationsList);
                    currentPage++;
                }
            }
        ...
        private void ReadResponse(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (info != null)
                {
                    ...
                    if (_apnsStream.CanRead)
                    {
                        ...
                        Logger.Error("Apple rejected palyload for device token : " +
                                     _notificationsList[payLoadIndex].DeviceToken);
                        Logger.Error("Apple Error code : " + _errorList[status]);
                        Logger.Error("Connection terminated by Apple.");
                        _rejected.Add(_notificationsList[payLoadIndex].DeviceToken);
                        _conected = false;
                    }
                }
            }
            ...
            {
                Logger.Error(
                    $"An error occurred while reading Apple response for token {_notificationsList[payLoadIndex].DeviceToken} - {ex.Message}");
            }
        }
        ...
        {
            Logger.Info("Creating SSL onnection.");
            ...
            try
            {
                ...
                if (_conected)
                {
                    ...
                    while (recd > 0)
                    {
                        ...
                        fb.Timestamp = new DateTime(2010, 1, 1, 0, 0, 0, DateTimeKind.Utc).AddSeconds(tSeconds).ToLocalTime();
                    ...
                    if (feedbacks.Count > 0)
                        Logger.Info($"Total {feedbacks.Count} feedbacks received.");
                    return feedbacks;
                }
            }
            catch (Exception ex)
